import type { OrderTemplate } from "./types";

export const EASY_ORDERS: OrderTemplate[] = [
  {
    orderNo: "4829",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 4 8 2 9. I want a pink round candy at zone \"A\" and a blue strawberry at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "round_candy", colour: "pink" },
      { zone: "D", foodItemId: "strawberry", colour: "blue" },
    ],
  },
  {
    orderNo: "7316",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 7 3 1 6. I want a blue star candy at zone \"B\" and a dark green strawberry at zone \"D\".",
    items: [
      { zone: "B", foodItemId: "star_candy", colour: "blue" },
      { zone: "D", foodItemId: "strawberry", colour: "dark green" },
    ],
  },
  {
    orderNo: "2058",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 2 0 5 8. I want beige cheese at zone \"C\" and a purple heart candy at zone \"A\".",
    items: [
      { zone: "C", foodItemId: "cheese", colour: "beige" },
      { zone: "A", foodItemId: "heart_candy", colour: "purple" },
    ],
  },
  {
    orderNo: "9143",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 9 1 4 3. I want yellow corn at zone \"A\", red tomato at zone \"B\", blue round candy at zone \"C\", and light green cheese at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "corn", colour: "yellow" },
      { zone: "B", foodItemId: "tomato", colour: "red" },
      { zone: "C", foodItemId: "round_candy", colour: "blue" },
      { zone: "D", foodItemId: "cheese", colour: "light green" },
    ],
  },
  {
    orderNo: "3671",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 3 6 7 1. I want a red triangle popsicle at zone \"C\" and a light green heart candy at zone \"A\".",
    items: [
      { zone: "C", foodItemId: "triangle_popsicle", colour: "red" },
      { zone: "A", foodItemId: "heart_candy", colour: "light green" },
    ],
  },
  {
    orderNo: "8562",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 8 5 6 2. I want a yellow flower candy at zone \"D\" and a purple lettuce at zone \"A\".",
    items: [
      { zone: "D", foodItemId: "flower_candy", colour: "yellow" },
      { zone: "A", foodItemId: "lettuce", colour: "purple" },
    ],
  },
  {
    orderNo: "1947",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 1 9 4 7. I want dark green capsicum at zone \"B\" and a pink spiral candy at zone \"C\".",
    items: [
      { zone: "B", foodItemId: "capsicum", colour: "dark green" },
      { zone: "C", foodItemId: "spiral_candy", colour: "pink" },
    ],
  },
  {
    orderNo: "6205",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 6 2 0 5. I want a black jellybean bear at zone \"A\" and red fries at zone \"B\".",
    items: [
      { zone: "A", foodItemId: "jellybean_bear", colour: "black" },
      { zone: "B", foodItemId: "fries", colour: "red" },
    ],
  },
  {
    orderNo: "7481",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 7 4 8 1. I want light blue carrot at zone \"D\", yellow cheese at zone \"B\", and red heart candy at zone \"A\".",
    items: [
      { zone: "D", foodItemId: "carrot", colour: "light blue" },
      { zone: "B", foodItemId: "cheese", colour: "yellow" },
      { zone: "A", foodItemId: "heart_candy", colour: "red" },
    ],
  },
  {
    orderNo: "3094",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 3 0 9 4. I want purple round popsicle at zone \"B\" and orange rectangular candy at zone \"D\".",
    items: [
      { zone: "B", foodItemId: "round_popsicle", colour: "purple" },
      { zone: "D", foodItemId: "rectangular_candy", colour: "orange" },
    ],
  },
  {
    orderNo: "5738",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 5 7 3 8. I want white cauliflower at zone \"C\" and yellow star candy at zone \"D\".",
    items: [
      { zone: "C", foodItemId: "cauliflower", colour: "white" },
      { zone: "D", foodItemId: "star_candy", colour: "yellow" },
    ],
  },
  {
    orderNo: "8260",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 8 2 6 0. I want light green corn at zone \"A\", blue tomato at zone \"C\", and yellow star candy at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "corn", colour: "light green" },
      { zone: "C", foodItemId: "tomato", colour: "blue" },
      { zone: "D", foodItemId: "star_candy", colour: "yellow" },
    ],
  },
  {
    orderNo: "4159",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 4 1 5 9. I want red mushroom at zone \"D\" and blue flower candy at zone \"B\".",
    items: [
      { zone: "D", foodItemId: "mushroom", colour: "red" },
      { zone: "B", foodItemId: "fries", colour: "blue" },
    ],
  },
  {
    orderNo: "2684",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 2 6 8 4. I want yellow corn at zone \"C\" and a orange rectangular candy at zone \"D\".",
    items: [
      { zone: "C", foodItemId: "corn", colour: "yellow" },
      { zone: "D", foodItemId: "rectangular_candy", colour: "orange" },
    ],
  },
  {
    orderNo: "9375",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 9 3 7 5. I want purple fries at zone \"A\" and dark green rectangular candy at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "fries", colour: "purple" },
      { zone: "D", foodItemId: "rectangular_candy", colour: "dark green" },
    ],
  },
  {
    orderNo: "5042",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 5 0 4 2. I want orange corn at zone \"B\", pink round popsicle at zone \"A\", and blue lettuce at zone \"C\".",
    items: [
      { zone: "B", foodItemId: "corn", colour: "orange" },
      { zone: "A", foodItemId: "round_popsicle", colour: "pink" },
      { zone: "C", foodItemId: "lettuce", colour: "blue" },
    ],
  },
  {
    orderNo: "6891",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 6 8 9 1. I want a brown round bun at zone \"C\" and a blue lettuce at zone \"A\".",
    items: [
      { zone: "C", foodItemId: "round_bun", colour: "brown" },
      { zone: "A", foodItemId: "lettuce", colour: "blue" },
    ],
  },
  {
    orderNo: "1426",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 1 4 2 6. I want red mushroom at zone \"D\" and yellow heart candy at zone \"B\".",
    items: [
      { zone: "D", foodItemId: "mushroom", colour: "red" },
      { zone: "B", foodItemId: "heart_candy", colour: "yellow" },
    ],
  },
  {
    orderNo: "7953",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 7 9 5 3. I want dark green dumpling at zone \"A\" and purple cheese at zone \"C\".",
    items: [
      { zone: "A", foodItemId: "dumpling", colour: "dark green" },
      { zone: "C", foodItemId: "cheese", colour: "purple" },
    ],
  },
  {
    orderNo: "3518",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 3 5 1 8. I want blue waffles at zone \"B\", red flower candy at zone \"C\", and yellow tomato at zone \"D\".",
    items: [
      { zone: "B", foodItemId: "waffles", colour: "blue" },
      { zone: "C", foodItemId: "flower_candy", colour: "red" },
      { zone: "D", foodItemId: "tomato", colour: "yellow" },
    ],
  },
  {
    orderNo: "6087",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 6 0 8 7. I want orange carrot at zone \"A\" and purple jellybean bear at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "carrot", colour: "orange" },
      { zone: "D", foodItemId: "jellybean_bear", colour: "purple" },
    ],
  },
  {
    orderNo: "9240",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 9 2 4 0. I want pink cinnamon roll at zone \"D\" and blue round candy at zone \"A\".",
    items: [
      { zone: "D", foodItemId: "cinnamon_roll", colour: "pink" },
      { zone: "A", foodItemId: "round_candy", colour: "blue" },
    ],
  },
  {
    orderNo: "2769",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 2 7 6 9. I want yellow mushroom at zone \"C\", light green star candy at zone \"B\", purple lettuce at zone \"A\", and red mushroom at zone \"D\".",
    items: [
      { zone: "C", foodItemId: "mushroom", colour: "yellow" },
      { zone: "B", foodItemId: "star_candy", colour: "light green" },
      { zone: "A", foodItemId: "lettuce", colour: "purple" },
      { zone: "D", foodItemId: "mushroom", colour: "red" },
    ],
  },
  {
    orderNo: "8306",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 8 3 0 6. I want a blue capsicum at zone \"B\" and a purple bread slices at zone \"D\".",
    items: [
      { zone: "B", foodItemId: "capsicum", colour: "blue" },
      { zone: "D", foodItemId: "bread_slices", colour: "purple" },
    ],
  },
  {
    orderNo: "4572",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 4 5 7 2. I want brown bread slices at zone \"D\" and pink rectangular candy at zone \"C\".",
    items: [
      { zone: "D", foodItemId: "bread_slices", colour: "brown" },
      { zone: "C", foodItemId: "rectangular_candy", colour: "pink" },
    ],
  },
  {
    orderNo: "1695",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 1 6 9 5. I want purple dumpling at zone \"A\", orange heart candy at zone \"B\", and light green tomato at zone \"C\".",
    items: [
      { zone: "A", foodItemId: "dumpling", colour: "purple" },
      { zone: "B", foodItemId: "heart_candy", colour: "orange" },
      { zone: "C", foodItemId: "tomato", colour: "light green" },
    ],
  },
  {
    orderNo: "7024",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 7 0 2 4. I want yellow fries at zone \"B\" and brown cheese at zone \"C\".",
    items: [
      { zone: "B", foodItemId: "fries", colour: "yellow" },
      { zone: "C", foodItemId: "cheese", colour: "brown" },
    ],
  },
  {
    orderNo: "3850",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 3 8 5 0. I want red cheese at zone \"C\" and blue fries at zone \"D\".",
    items: [
      { zone: "C", foodItemId: "cheese", colour: "red" },
      { zone: "D", foodItemId: "fries", colour: "blue" },
    ],
  },
  {
    orderNo: "9517",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 9 5 1 7. I want dark green cauliflower at zone \"A\" and brown spiral candy at zone \"C\".",
    items: [
      { zone: "A", foodItemId: "cauliflower", colour: "dark green" },
      { zone: "C", foodItemId: "spiral_candy", colour: "brown" },
    ],
  },
  {
    orderNo: "2468",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 2 4 6 8. I want pink corn at zone \"D\", yellow triangle popsicle at zone \"B\", blue lettuce at zone \"A\", and red round candy at zone \"C\".",
    items: [
      { zone: "D", foodItemId: "corn", colour: "pink" },
      { zone: "B", foodItemId: "triangle_popsicle", colour: "yellow" },
      { zone: "A", foodItemId: "lettuce", colour: "blue" },
      { zone: "C", foodItemId: "round_candy", colour: "red" },
    ],
  },
  {
    orderNo: "6173",
    difficulty: "easy",
    customerSlot: 1,
    spokenText:
      "Order number 6 1 7 3. I want orange dumpling at zone \"C\" and pink strawberry at zone \"A\".",
    items: [
      { zone: "C", foodItemId: "dumpling", colour: "orange" },
      { zone: "A", foodItemId: "strawberry", colour: "pink" },
    ],
  },
  {
    orderNo: "4908",
    difficulty: "easy",
    customerSlot: 2,
    spokenText:
      "Order number 4 9 0 8. I want blue noodles at zone \"A\" and purple star candy at zone \"D\".",
    items: [
      { zone: "A", foodItemId: "noodles", colour: "blue" },
      { zone: "D", foodItemId: "star_candy", colour: "purple" },
    ],
  },
  {
    orderNo: "8635",
    difficulty: "easy",
    customerSlot: 3,
    spokenText:
      "Order number 8 6 3 5. I want light green mushroom at zone \"B\", yellow rectangular candy at zone \"C\", and pink cheese at zone \"D\".",
    items: [
      { zone: "B", foodItemId: "mushroom", colour: "light green" },
      { zone: "C", foodItemId: "rectangular_candy", colour: "yellow" },
      { zone: "D", foodItemId: "cheese", colour: "pink" },
    ],
  },
  {
    orderNo: "1287",
    difficulty: "easy",
    customerSlot: 4,
    spokenText:
      "Order number 1 2 8 7. I want brown round bun at zone \"A\" and red heart candy at zone \"B\".",
    items: [
      { zone: "A", foodItemId: "round_bun", colour: "brown" },
      { zone: "B", foodItemId: "heart_candy", colour: "red" },
    ],
  },
  {
    orderNo: "7349",
    difficulty: "easy",
    customerSlot: 5,
    spokenText:
      "Order number 7 3 4 9. I want orange jellybean bear at zone \"C\" and blue round bun at zone \"A\".",
    items: [
      { zone: "C", foodItemId: "jellybean_bear", colour: "orange" },
      { zone: "A", foodItemId: "round_bun", colour: "blue" },
    ],
  },
  {
    orderNo: "5601",
    difficulty: "easy",
    customerSlot: 6,
    spokenText:
      "Order number 5 6 0 1. I want purple carrot at zone \"D\" and dark green flower candy at zone \"A\".",
    items: [
      { zone: "D", foodItemId: "carrot", colour: "purple" },
      { zone: "A", foodItemId: "flower_candy", colour: "dark green" },
    ],
  },
];
